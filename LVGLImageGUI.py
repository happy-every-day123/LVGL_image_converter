...
import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QLabel, QFileDialog, QVBoxLayout, QWidget, QComboBox, QSpinBox, QCheckBox, QLineEdit, QProgressBar
...
class LVGLImageConverterApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.input_button = QPushButton('Select Folder', self)
        self.input_button.clicked.connect(self.select_input_file)
        layout.addWidget(self.input_button)
        self.input_path_label = QLabel('No Folder selected', self)
        ...
        self.output_path_label = QLabel('No directory selected', self)
        layout.addWidget(self.output_path_label)

        # 颜色格式选择
        self.cf_label = QLabel('Color Format:', self)
        layout.addWidget(self.cf_label)
        self.cf_combobox = QComboBox(self)
        self.cf_combobox.addItems([
            "L8", "I1", "I2", "I4", "I8", "A1", "A2", "A4", "A8",
            "ARGB8888", "XRGB8888", "RGB565", "RGB565A8", "RGB888",
            "TRUECOLOR", "TRUECOLOR_ALPHA", "AUTO"
        ])
        layout.addWidget(self.cf_combobox)

        # 压缩方法选择
        self.compress_label = QLabel('Compression Method:', self)
        layout.addWidget(self.compress_label)
        self.compress_combobox = QComboBox(self)
        self.compress_combobox.addItems(["NONE", "RLE", "LZ4"])
        layout.addWidget(self.compress_combobox)
        
        # 目标格式选择
        self.target_format = QLabel('Target Format:', self)
        layout.addWidget(self.target_format)
        self.target_format_combobox = QComboBox(self)
        self.target_format_combobox.addItems(["BIN", "C", "PNG"])
        layout.addWidget(self.target_format_combobox)

        # 对齐选择
        self.align_label = QLabel('Stride Alignment (bytes):', self)
        layout.addWidget(self.align_label)
        self.align_spinbox = QSpinBox(self)
        self.align_spinbox.setMinimum(1)
        self.align_spinbox.setMaximum(1024)
        self.align_spinbox.setValue(1)
        layout.addWidget(self.align_spinbox)

        # 背景颜色选择
        self.background_label = QLabel('Background Color (hex):', self)
        layout.addWidget(self.background_label)
        self.background_input = QLineEdit(self)
        self.background_input.setPlaceholderText("e.g., 0x000000")
    ...
    def select_input_file(self):
        file_path = QFileDialog.getExistingDirectory(self, 'Select Input Folder')
        if file_path != '':
    ...
    def convert_images(self):
        input_file = self.input_path_label.text()
        output_dir = self.output_path_label.text()
        color_format = self.cf_combobox.currentText()
        compression_method = self.compress_combobox.currentText()
        stride_alignment = self.align_spinbox.value()
        ofmt = self.target_format_combobox.currentText()
        if self.background_input.text() == '':
            background_color = 0x000000
        else:
            background_color = self.background_input.text()

        if not isinstance(background_color, int):
            background_color=int(background_color, 16)
        # 调用主函数进行转换
        try:
            result = main(files=input_file, ofmt=ofmt, cf=color_format, compress=compression_method, align=stride_alignment, background=0x000000, output=output_dir)
            self.result_label.setText(f'Conversion completed successfully.')
        except Exception as e:
            self.result_label.setText(str(e))

class PNGConverter(LVGLImageConverterApp):
    ...
    def __init__(self,
                 ...
                 keep_folder=True,
                 parent=None) -> None:
        super(PNGConverter, self).__init__(parent)
        self.files = files
        self.cf = cf
        self.ofmt = ofmt
        self.output = odir
    ...
    def convert(self):
        ...
        for i, f in enumerate(self.files):
            ...
            sys.stdout.write(f"converted {i+1}/{len(self.files)} files: {f}\n")

        return output

def main(files, ofmt, cf, compress, align, output, background=0x000000):
    
    cf = ColorFormat[cf]
    files = list(Path(files).rglob("*.[pP][nN][gG]"))
    ofmt = OutputFormat(ofmt)
    compress = CompressMethod[compress]
    
    logging.basicConfig(level=logging.WARN)
    # logging.info(f"options: {__dict__}, files:{[str(f) for f in files]}")
...
if __name__ == '__main__':
    ...
    main_window = LVGLImageConverterApp()
    main_window.show()
    sys.exit(app.exec_())